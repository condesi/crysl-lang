...
\title{%
  \textbf{QOMN: A Domain-Specific Language for Deterministic}\\
...
\author{
  ...
  \href{https://github.com/condesi/qomn-lang}{github.com/condesi/qomn-lang} \quad
  \href{https://desarrollador.xyz/benchmark.html}{Live Demo: desarrollador.xyz/benchmark.html}
}
\date{April 2026}
...
\begin{abstract}
We present \textbf{QOMN} (QOMN Language), an open domain-specific language
...
A QOMN program describes \emph{what to compute} — the runtime handles numeric
dispatch, unit validation, and result formatting.

We show that QOMN achieves \textbf{53--286~ms end-to-end latency} for
...
probabilistic approximations. We compare against GPT-4 Turbo, a second commercial LLM API,
Python/SciPy scripts, Excel formulas, and commercial engineering software.

QOMN v2 is released as an open standard under MIT license. The language
grammar, standard library (6 domains, 12 plans), WASM runtime, and contribution
guide are available at \href{https://github.com/condesi/qomn-lang}{condesi/qomn-lang}.
\end{abstract}
...
QOMN addresses all three. A QOMN \texttt{plan\_pump\_sizing} program
...
A QOMN program consists of one or more \textbf{plan declarations}.
Each plan has:
...
\begin{lstlisting}[caption=QOMN v2 formal grammar]
program    ::= plan_decl+
...
\begin{table}[h]
\centering
\caption{QOMN v2 built-in function library}
\begin{tabular}{lll}
\toprule
...
\begin{enumerate}
  \item \textbf{Lexer}: tokenizes QOMN source into keyword, ident, literal, op tokens
...
QOMN compiles to standard WebAssembly, runnable in:
\begin{itemize}
  \item \textbf{Browser}: via the QOMN JIT web interface at
    \href{https://desarrollador.xyz/benchmark.html}{desarrollador.xyz/benchmark.html}
...
%─────────────────────────────────────────────────────────────────────────────
\section{Benchmark: QOMN vs LLM vs Python vs Excel}
...
\midrule
\textbf{QOMN JIT (Qomni)} & \textbf{49} & \textbf{302} & Server5, 5 runs avg \\
...
LLM B (API) & 2,200 & 7,000 & Commercial API, March 2026 \\
Local LLM 1.5B (Qomni) & 8,000 & 45,000 & qwen2.5-1.5b-instruct-q4 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Measured QOMN Benchmark (Server5, 2026-04-13)}

\begin{table}[h]
\centering
\caption{5-run QOMN benchmark — Server5 (AMD EPYC 12-core)}
...
\midrule
\textbf{QOMN JIT} & \textbf{13.26 HP} & \textbf{Yes} & \textbf{NFPA 20:2022 §4.26} & \textbf{Yes} \\
GPT-4 Turbo & ``about 15 HP'' & No ($\pm$15\%) & No & No \\
LLM B (API) & ``13-14 HP'' & Partial & No & No \\
...
QOMN, Python, and Excel all produce the exact answer, but only QOMN
...
\toprule
& QOMN & GPT-4 & LLM-B & Python & Excel & Eng.~SW \\
\midrule
...
QOMN is not a replacement for LLMs on unstructured or creative tasks.
LLMs outperform QOMN when:
\begin{itemize}
  ...
  \item The calculation is novel and not in the QOMN stdlib
\end{itemize}

The optimal architecture (implemented in Qomni Engine) uses both:
QOMN handles the deterministic calculation layer, the LLM handles
...
\begin{table}[h]
\centering
\caption{QOMN v2 domain registry}
\begin{tabular}{lll}
\toprule
...
QOMN v2.1 is live and accessible at:
\begin{center}
  \href{https://desarrollador.xyz/benchmark.html}{\textbf{desarrollador.xyz/benchmark.html}}
\end{center}
...
QOMN v2 provides a principled middle ground between the rigidity of
...
The QOMN language, standard library, runtime, and this paper are released
under MIT license at \href{https://github.com/condesi/qomn-lang}{condesi/qomn-lang}.
